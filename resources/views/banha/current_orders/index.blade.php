        ...
        <div class="card-datatable table-responsive pt-0">
            <table id="dataTable" class="datatables-basic table">
                ...
                <tr>
                    ...
                    <th>{{__('banha.payment_type')}}</th>
                    <th>{{__('banha.total_points')}}</th>
    ...
    <script>
        ...
        $(function () {
            myTable = $('#dataTable').DataTable({
                ...
                columns: [
                    ...
                    {"data": 'driver_name', name: 'driver_id', orderable: false, searchable: true},
        ...
        $(document).ready(function () {
            ...
            function updateOrderStatus(id, status, successText) {
                $.ajax({
                    url: "{{ route('update-order-status.update', ':id') }}".replace(":id", id),
                    type: "put",
                    data: {
                        _token: "{{ csrf_token() }}",
                        status: status,
            ...
            $(document).on("click", ".choose_driver", function () {
                let id = $(this).data('id')
                let url = "{{route('get-available-drivers.index')}}"
                $.ajax({
                    url: url,
                    type: "GET",
                    data: {
