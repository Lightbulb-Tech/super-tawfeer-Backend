<form id="form" action="{{$updateRoute}}" method="post">
    ...
    <div class="row">
        <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control" name="code" data-validation="required" value="{{$obj->code}}"
                   id="generatedCode">
        </div>
        ...
        <div class="col-4 mb-3">
            ...
            <select class="form-control" name="type" data-validation="required">
                ...
                @foreach($couponsTypes as $couponsType)
                    <option value="{{$couponsType->value}}" @selected($couponsType->value == $obj->type)>
        ...
        <div class="col-4 mb-3">
            ...
            <input type="number" class="form-control" min="1" name="value" value="{{$obj->value}}"
                   data-validation="required">
        </div>
        <div class="col-4 mb-3">
            <label for="value" class="form-label">{{__('banha.usage_times')}}</label>
            <input type="number" class="form-control" min="1" name="usage_times" value="{{$obj->usage_times}}"
                   data-validation="required">
        </div>
        <div class="col-6 mb-3">
            <label for="from_date" class="form-label">{{__('banha.from_date')}}</label>
            <input type="date" class="form-control" name="from_date" value="{{$obj->from_date}}"
                   data-validation="required">
        </div>

        <div class="col-6 mb-3">
            <label for="to_date" class="form-label">{{__('banha.to_date')}}</label>
            <input type="date" class="form-control" name="to_date" value="{{$obj->to_date}}" data-validation="required">
        </div>
