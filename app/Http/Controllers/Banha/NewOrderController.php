...
{
    private string $folderPath = 'banha.new_orders.';
    private string $mainRoute = 'new-orders';

    public function index(Request $request, objService $service)
    {
        if ($request->ajax()) {
            $data = $service->getOrdersForDataTable([OrderStatusEnum::Pending->value]);
...
//                    return '<a href="' . route('invoices.show', $row->id) . '">' . __('banha.download_invoice') . '</a>';
//                })
                ...
                ->editColumn('status', function ($row) {
                    ...
                    if ($row->status == OrderStatusEnum::Pending->value) {
                        ...
                        <div class="btn-group" role="group">
                            <button class="btn btn-success btn-sm accept-transfer" data-id="' . $row->id . '" data-order-id="' . @$row->id . '">
                                <i class="fas fa-check"></i> ' . __('banha.accept') . '
                            </button>
                            <button class="btn btn-danger btn-sm reject-transfer mx-2" data-id="' . $row->id . '" data-order-id="' . @$row->id . '">
...
//                    $deleteButton = deleteBtn($this->mainRoute, $row->id);
                ...
                ->rawColumns(['actions', 'status', 'final_price', 'total_points', 'payment_type', 'invoice_pdf'])
                ->make(true);
        }
        $data["createRoute"] = route($this->mainRoute . ".create");
        $data["dataTableRoute"] = route($this->mainRoute . ".index");
        $data["bladeTitle"] = __("banha.new_orders");
        $data["addButtonText"] = __("banha.new_orders");
        $data["modelSize"] = 'xl';
