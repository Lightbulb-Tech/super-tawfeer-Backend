...
use App\Http\Requests\Banha\CouponRequest as objRequest;
...
{
    private string $folderPath = 'banha.coupons.';
    private string $mainRoute = 'coupons';
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->editColumn('type', function ($row) {
                    return CouponTypeisEnum::tryFrom($row->type)->lang();
                })
                ->editColumn('code', function ($row) {
                    return '<span class="badge bg-danger">'.$row->code.'</span>';
                })
                ...
                ->rawColumns(['actions', 'is_active', 'type','code'])
                ->make(true);
        }
        $data["createRoute"] = route($this->mainRoute . ".create");
        $data["dataTableRoute"] = route($this->mainRoute . ".index");
        $data["bladeTitle"] = __("banha.coupons");
        $data["addButtonText"] = __("banha.coupons");
        $data["modelSize"] = 'xl';
        ...
        if (request()->ajax()) {
            $html = view($this->folderPath . 'create')
                ->with([
                    ...
                    'couponsTypes' => CouponTypeisEnum::cases(),
                ])->render();
