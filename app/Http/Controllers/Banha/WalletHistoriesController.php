<?php

namespace App\Http\Controllers\Banha;

use App\Enums\PointHistoryStatusEnum;
use App\Enums\WalletOperationEnum;
use App\Enums\WalletStatusEnum;
use App\Http\Controllers\Controller;
use App\Services\Banha\WalletHistoryService as objService;
use Illuminate\Http\Request;
use Yajra\DataTables\Facades\DataTables;

class WalletHistoriesController extends Controller
{
    private string $folderPath = 'banha.wallet_histories.';
    private string $mainRoute = 'wallet-histories';

    public function index(Request $request, objService $service)
    {
        if ($request->ajax()) {
            $data = $service->getDataTable();
            return DataTables::of($data)
                ->addIndexColumn()
                ->editColumn('user', function ($row) {
                    return @$row->user->first_name ?? '-' . ' ' . $row->user->last_name;
                })
                ->editColumn('user_phone', function ($row) {
                    return @$row->user->phone;
                })
                ->editColumn('status', function ($row) {
                    $result = '';
                    if ($row->status == WalletStatusEnum::Pending->value) {
                        $result = '<span class="badge bg-warning">' . WalletStatusEnum::tryFrom($row->status)->lang() . '</span>';
                    } else if ($row->status == WalletStatusEnum::Success->value) {
                        $result = '<span class="badge bg-success">' . WalletStatusEnum::tryFrom($row->status)->lang() . '</span>';
                    } else if ($row->status == WalletStatusEnum::Failed->value) {
                        $result = '<span class="badge bg-danger">' . WalletStatusEnum::tryFrom($row->status)->lang() . '</span>';
                    }
                    return $result;
                })
                ->editColumn('operation', function ($row) {
                    return WalletOperationEnum::tryFrom($row->operation)->lang();
                })
                ->editColumn('amount', function ($row) {
                    return $row->amount;
                })
                ->addColumn('actions', function ($row) {
                    $editButton = '';
//                    $editButton = editBtn($this->mainRoute, $row->id, $row->title);
                    $deleteButton = deleteBtn($this->mainRoute, $row->id);
                    return $editButton . ' ' . $deleteButton;
                })
                ->rawColumns(['actions', 'user', 'status', ])
                ->make(true);
        }
        $data["createRoute"] = route($this->mainRoute . ".create");
        $data["dataTableRoute"] = route($this->mainRoute . ".index");
        $data["bladeTitle"] = __("banha.wallet_histories");
        $data["addButtonText"] = __("banha.wallet_histories");
        $data["modelSize"] = 'xl';
        return view($this->folderPath . '.index', $data);
    }

    public function destroy(string $id, objService $service)
    {
        $service->deleteWithFiles($id);
        return jsonSuccess();

    }
}
